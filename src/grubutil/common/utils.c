...
struct master_and_dos_boot_sector {
  ...
  /* 0B */ unsigned short bytes_per_sector;/* seems always to be 512, so we just use 512 */
  /* 0D */ unsigned char sectors_per_cluster;/* non-zero, the power of 2, i.e., 2^n */
  /* 0E */ unsigned short reserved_sectors;/* FAT=non-zero, NTFS=0? */
  /* 10 */ unsigned char number_of_fats;/* NTFS=0; FAT=1 or 2  */
  /* 11 */ unsigned short root_dir_entries;/* FAT32=0, NTFS=0, FAT12/16=non-zero */
  /* 13 */ unsigned short total_sectors_short;/* FAT32=0, NTFS=0, FAT12/16=any */
  /* 15 */ unsigned char media_descriptor;/* range from 0xf0 to 0xff */
  /* 16 */ unsigned short sectors_per_fat;/* FAT32=0, NTFS=0, FAT12/16=non-zero */
  /* 18 */ unsigned short sectors_per_track;/* range from 1 to 63 */
  /* 1A */ unsigned short total_heads;/* range from 1 to 256 */
  /* 1C */ unsigned long hidden_sectors;/* any value */
  /* 20 */ unsigned long total_sectors_long;/* FAT32=non-zero, NTFS=0, FAT12/16=any */
  /* 24 */ unsigned long sectors_per_fat32;/* FAT32=non-zero, NTFS=any, FAT12/16=any */
  /* 28 */ unsigned long total_sectors_long_1;/* NTFS=non-zero, FAT12/16/32=any */
  /* 28 */ unsigned long total_sectors_long_2;/* NTFS=non-zero, FAT12/16/32=any */
  ...
  /* 1FE */ unsigned short boot_signature;/* 0xAA55 */
#if 0
  ...
  /* 220 */ long long L[9]; /* L[n] == LBA[n] - S[n] + 1 */
  /* 268 */ long H[9];
  /* 28C */ short C[9];
  /* 29E */ short X;
  /* 2A0 */ short Y;
      ...
      if (buf[i+4])
        {
          ...
          for (j=0;j<pn;j++)
            if ((pt[pn][0]>pt[j][0])?(pt[pn][0]<pt[j][1]):(pt[pn][1]>pt[j][0]))
              return 0;
