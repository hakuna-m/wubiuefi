...
#ifdef WIN32
  hd=(int) CreateFile(fn, ((rdwr)?GENERIC_WRITE:0) | GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
                      NULL,OPEN_EXISTING, FILE_FLAG_WRITE_THROUGH | FILE_FLAG_NO_BUFFERING,0);
#else
...
int xd_seek(xd_t* xd,unsigned long sec)
{
  xd->ofs=sec;
  if (xd->flg==XDF_FILE)
    {
...
int xd_read(xd_t* xd,char *buf,int len)
{
  int nr;
...
#ifdef WIN32
  {
    ...
    if ((! ReadFile((HANDLE)xd->num, buf, nr, &nn, NULL)) || (nr != (int) nn))
      return 1;
  }
#else
...
int xd_write(xd_t* xd,char *buf,int len)
{
  int nr;
...
#ifdef WIN32
  {
    ...
    if ((! WriteFile((HANDLE)xd->num, buf, nr, &nn, NULL)) || (nr != (int) nn))
      return 1;
  }
#else
