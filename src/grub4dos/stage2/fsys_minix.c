...
#define SUPERBLOCK \
    ((struct minix_super_block *)((FSYS_BUF)+512))
#define INODE \
...
{
  unsigned long current_ino = MINIX_ROOT_INO;  /* start at the root */
  unsigned long updir_ino = current_ino;     /* the parent of the current directory */
  unsigned long ino_blk;	     /* fs pointer of the inode's info */

  unsigned long str_chk = 0;	     /* used ot hold the results of a string
				        compare */
...
//char linkbuf[PATH_MAX];	     /* buffer for following sym-links */
  ...
  unsigned long off;		     /* offset within block of directory 
					entry */
  unsigned long loc;		     /* location within a directory */
  unsigned long blk;		     /* which data blk within dir entry */
  unsigned long map;		     /* fs pointer of a particular block from
					dir entry */
...
//	   rest++);
      ...
      do
	{
	  ...
	  if (dp->inode)
	    {
	      //unsigned long saved_c = dp->name[namelen];
	      int j, k;
	      char ch1;
	      char *tmp_name= NAME_BUF;	/* NAME_LEN is 255, so 512 byte buffer is needed. */

	      /* copy dp->name to tmp_name, and quote spaces with '\\' */
