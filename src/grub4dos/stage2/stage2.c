	  ...
	  char *heap, int entryno)
{
  ...
  for (i = max_len = 0; i < num_entries; i++)
    {
      j = strlen(get_entry(menu_entries, i));
      if (j > max_len)
	max_len = j;
    }
  ...
  for(i = 0; i < (int) gfx1->menu_entries; i++)
    {
      /* Skip the leading '\t'.  */
      strcpy (gfx1->menu_list + i * gfx1->menu_entry_len, get_entry(menu_entries, i) + 1);
    }
  ...
  gfx_file_size = grub_read(buf_ext, buf_ext_size, 0xedde0d90);
