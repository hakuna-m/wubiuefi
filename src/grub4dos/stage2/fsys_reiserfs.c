	  ...
	  if (j_len >= JOURNAL_TRANS_HALF)
	    {
	      ...
	      if (! journal_read (commit_block, 
				  sizeof (struct reiserfs_journal_commit), (char *) commit))
		return 0;
	      while (i < j_len)
...
journal_init (void)
{
  ...
  while (1) 
    {
      journal_read (desc_block, sizeof (struct reiserfs_journal_desc), (char *) desc);
      if (substring (JOURNAL_DESC_MAGIC, desc->j_magic, 0) > 0
	...
	break;
      ...
      journal_read (commit_block, sizeof (struct reiserfs_journal_commit), (char *) commit);
...
reiserfs_mount (void)
{
  ...
  INFO->fullblocksize_shift = log2_tmp (super->s_blocksize);
